...
const Navbar = () => {
  return (
    <header className="text-gray-800 bg-white shadow-sm fixed w-full z-30">
      <nav className="max-w-6xl mx-auto flex flex-col items-center justify-between p-4 sm:flex-row">
        <div>
          <Link
            to="/"
            className="flex items-center justify-center gap-2 text-2xl font-bold hover:text-gray-500"
          >
            <svg
              fill="#4F46E5"
            ...
            {links.map(({ id, text, href }) => {
              return (
                <Link
                  key={id}
                  to={href}
                  className="text-sm sm:text-lg font-semibold hover:text-gray-400"
                >
                  {text}
                </Link>
              );
